...
def test_moderate_chain():
    ...
    for q_a_pair in q_a_pairs:
        result, traces = ch.perform_monitor(
...
def test_database_answering_chain():
    ...
    uri = f"postgresql+psycopg2://{config('DBUSERNAME')}:\
{config('DBPASSWORD')}@{config('DBHOST')}:{config('DBPORT')}/postgres"
    ch = DatabaseAnswerChain.from_llamacpp_config(
        prompt_name="answer_database_short",
        uri=uri,
        include_tables=[
            'tb_patient',
            # 'tb_patients_allergies',
    ...
    result, traces = ch.perform_monitor(
                user_input="Show me the top 5 frequently used medicine",
                get_cmd=True,
                get_db=False,
                get_summary=True)
